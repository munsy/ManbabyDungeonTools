...
function NDT:POI_CreateFramePools()
    NDT.poi_framePools = NDT.poi_framePools or CreateFramePoolCollection()
    NDT.poi_framePools:CreatePool("Button", NDT.main_frame.mapPanelFrame, "MapLinkPinTemplate")
    NDT.poi_framePools:CreatePool("Button", NDT.main_frame.mapPanelFrame, "DeathReleasePinTemplate")
    NDT.poi_framePools:CreatePool("Button", NDT.main_frame.mapPanelFrame, "VignettePinTemplate")
    NDT.poi_framePools:CreatePool("Frame", NDT.main_frame.mapPanelFrame, "NDTAnimatedLineTemplate")
end
...
local function POI_SetDevOptions(frame,poi)
    ...
    frame:SetScript("OnMouseDown", function(self, button)
        ...
        if button == "RightButton" then
            local pois = NDT.mapPOIs[db.currentDungeonIdx][NDT:GetCurrentSubLevel()]
...
local function POI_SetOptions(frame,type,poi,homeSublevel)
    ...
    if type == "mapLink" then
        ...
        frame:SetScript("OnClick",function()
            NDT:SetCurrentSubLevel(poi.target)
            NDT:UpdateMap()
            NDT:ZoomMapToDefault()
        end)
        frame:SetScript("OnEnter",function()
            GameTooltip:SetOwner(UIParent, "ANCHOR_CURSOR")
            GameTooltip:AddLine(NDT:GetDungeonSublevels()[db.currentDungeonIdx][poi.target], 1, 1, 1, 1)
    ...
    if type == "nyalothaSpire" then
        ...
        frame:SetScript("OnMouseUp",function(self,button)
            if button == "RightButton" then
                --reset npc location
                NDT:GetRiftOffsets()[self.npcId]=nil
                NDT:UpdateMap()
                if NDT.liveSessionActive and NDT:GetCurrentPreset().uid == NDT.livePresetUID then
                    NDT:LiveSession_SendCorruptedPositions(NDT:GetCurrentPreset().value.riftOffsets)
                end
            end
            if button == "LeftButton" then
                local _, connections = NDT:FindConnectedDoor(frame.npcId,1)
                if connections then
                    NDT:SetCurrentSubLevel(connections[#connections].target)
                    NDT:UpdateMap()
                end
            end
        end)
        ...
        frame:SetScript("OnEnter",function()
            ...
            local blips = NDT:GetDungeonEnemyBlips()
                ...
                for _,blip in pairs(blips) do
                    if blip.data.id == poi.npcId then
                        ...
                        if isBlipSameWeek then
                            ...
                            frame.animatedLine = NDT:ShowAnimatedLine(NDT.main_frame.mapPanelFrame,frame,blipFrame,nil,nil,nil,nil,nil,not frame.isSpire,frame.animatedLine)
            ...
            local connectedDoor, connections = NDT:FindConnectedDoor(frame.npcId,1)
            if connectedDoor then
                if frame.isSpire then
                    frame.animatedLine = NDT:ShowAnimatedLine(NDT.main_frame.mapPanelFrame,frame,connectedDoor,nil,nil,nil,nil,nil,not frame.isSpire,frame.animatedLine)
                end
                local sublevelName = NDT:GetSublevelName(nil,connections[#connections].target)
                local npcName = NDT:GetNPCNameById(frame.npcId)
        ...
        frame:SetScript("OnLeave",function()
            ...
            if frame.isSpire then
                NDT:HideAnimatedLine(frame.animatedLine)
            end
        end)
        --check expanded status
        if NDT:IsNPCInPulls(poi) then
...
function NDT:POI_HideAllPoints()
...
---POI_UpdateAll
function NDT:POI_UpdateAll()
    twipe(points)
    db = NDT:GetDB()
    local framePools = NDT.poi_framePools
    ...
    if not NDT.mapPOIs[db.currentDungeonIdx] then return end
    local currentSublevel = NDT:GetCurrentSubLevel()
    local pois = NDT.mapPOIs[db.currentDungeonIdx][currentSublevel]
    if not pois then return end
    local preset = NDT:GetCurrentPreset()
    local teeming = NDT:IsPresetTeeming(preset)
    local scale = NDT:GetScale()
    local week = NDT:GetEffectivePresetWeek(preset)
    for poiIdx,poi in pairs(pois) do
        ...
        then
            ...
            poiFrame:SetPoint("CENTER", NDT.main_frame.mapPanelTile1,"TOPLEFT",poi.x*scale,poi.y*scale)
...
local function animateLine(self, elapsed)
    ...
    for i=1,numLines + 1 do
        ...
        tex:SetPoint("TOPLEFT", NDT.main_frame.mapPanelTile1,"TOPLEFT",tX - (self.sizeX / 2),tY - (self.sizeY / 2))
        tex:SetPoint("BOTTOMRIGHT", NDT.main_frame.mapPanelTile1,"TOPLEFT",tX + (self.sizeX / 2),tY + (self.sizeY / 2))
...
local function createAnimatedLine(parent)
    local animatedLine = NDT.poi_framePools:Acquire("NDTAnimatedLineTemplate")
    animatedLine:Show()
...
function NDT:ShowAnimatedLine(parent, frame1, frame2, sizeX, sizeY, gap, color, speed, selected, animatedLine)
    if not (frame1 and frame2 and (not frame1:IsForbidden()) and (not frame1:IsForbidden())) then
        return nil
    end
    texturePool = texturePool or CreateTexturePool(NDT.main_frame.mapPanelFrame ,"OVERLAY",7,nil)
    ...
    local scale = NDT:GetScale()
    local mapSizex,mapSizey = NDT:GetDefaultMapPanelSize()
...
function NDT:KillAllAnimatedLines()
    local linePool = self.poi_framePools:GetPool("NDTAnimatedLineTemplate")
...
function NDT:DrawAllAnimatedLines()
    local week = self:GetEffectivePresetWeek()
    for _,blip in pairs(NDT:GetDungeonEnemyBlips()) do
        if not blip:IsShown() and blip.data.corrupted then
            NDT:HideAnimatedLine(blip.animatedLine)
        elseif blip.data.corrupted and blip.selected then
            ...
            local connectedDoor = NDT:FindConnectedDoor(blip.data.id)
            connectedFrame = connectedDoor or connectedFrame
            blip.animatedLine = NDT:ShowAnimatedLine(NDT.main_frame.mapPanelFrame,connectedFrame,blip,nil,nil,nil,nil,nil,blip.selected)
    ...
    local _,activeSpires = NDT.poi_framePools:GetPool("VignettePinTemplate"):EnumerateActive()
    for poiFrame,_ in pairs(activeSpires) do
        if poiFrame.spireIndex and poiFrame.npcId and not poiFrame.isSpire and not poiFrame.animatedLine then
            local connectedDoor = NDT:FindConnectedDoor(poiFrame.npcId,1)
            if connectedDoor then
                poiFrame.animatedLine = NDT:ShowAnimatedLine(NDT.main_frame.mapPanelFrame,poiFrame,connectedDoor,nil,nil,nil,nil,nil,not poiFrame.isSpire)
            end
        end
    end
end

function NDT:HideAnimatedLine(animatedLine)
...
function NDT:FindConnectedDoor(npcId, numConnection)
