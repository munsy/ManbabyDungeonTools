...
function NDT:GetDungeonEnemyBlips()
    return blips
end
...
function NDT:DoFramesOverlap(frameA, frameB, offset)
...
function NDTDungeonEnemyMixin:OnEnter()
    ...
    if self.data.corrupted then
        ...
        if not self.selected then
            local _,active = NDT.poi_framePools:GetPool("VignettePinTemplate"):EnumerateActive()
            for poiFrame,_ in pairs(active) do
                if poiFrame.spireIndex and poiFrame.npcId == self.data.id then
                    ...
                    self.animatedLine = NDT:ShowAnimatedLine(NDT.main_frame.mapPanelFrame,self.spireFrame,self,nil,nil,nil,nil,nil,self.selected,self.animatedLine)
                    self.spireFrame.animatedLine = self.animatedLine
                    break
                end
            end
            local connectedDoor = NDT:FindConnectedDoor(self.data.id)
            if connectedDoor then
                self.animatedLine = NDT:ShowAnimatedLine(NDT.main_frame.mapPanelFrame,connectedDoor,self,nil,nil,nil,nil,nil,self.selected,self.animatedLine)
            end
        end
    end
    if not db.devMode then
        self.fontstring_Text1:SetText(NDT:IsCurrentPresetTeeming() and self.data.teemingCount or self.data.count)
        ...
        for _,blip in pairs(blips) do
            if blip.clone.g == self.clone.g then
                blip.fontstring_Text1:SetText(NDT:IsCurrentPresetTeeming() and blip.data.teemingCount or blip.data.count)
...
function NDTDungeonEnemyMixin:OnLeave()
    self:updateSizes(1)
    self:SetFrameLevel(self:GetFrameLevel()-5)
    if db.devMode then
    ...
    NDT:DisplayBlipTooltip(self,false)
    if self.data.corrupted then
        ...
        if not self.selected then
            NDT:HideAnimatedLine(self.animatedLine)
        end
    end
    if not db.devMode then
        if not self.clone.g then
...
local function setUpMouseHandlers(self)
    ...
    self:SetScript("OnDragStart", function()
        local x,y,scale
        preset = NDT:GetCurrentPreset()
        tempPulls = NDT:DeepCopy(preset.value.pulls)
        local targetPull
        local _, _, _, blipX, blipY = self:GetPoint()
        self:SetScript("OnUpdate",function()
            local nx,ny = NDT:GetCursorPosition()
            if x~=nx or y~=ny then
                x,y = nx,ny
                --find closest pull and measure distance
                local pullIdx,centerX,centerY = NDT:FindClosestPull(x,y)
                ...
                if not isClose then
                    targetPull = nil
                    NDT:DungeonEnemies_AddOrRemoveBlipToCurrentPull(self, false, IsControlKeyDown(),tempPulls,nil,true)
                    NDT:DungeonEnemies_UpdateSelected(NDT:GetCurrentPull(),tempPulls)
                elseif pullIdx~=targetPull then
                    targetPull = pullIdx
                    NDT:DungeonEnemies_AddOrRemoveBlipToCurrentPull(self, true, IsControlKeyDown(),tempPulls,pullIdx,true)
                    NDT:DungeonEnemies_UpdateSelected(NDT:GetCurrentPull(),tempPulls)
                end
            end
        end)
...
local function setUpMouseHandlersAwakened(self,clone,scale,riftOffsets)
    ...
    self:SetScript("OnMouseDown",function(self, button)
        if button == "LeftButton" then
            riftOffsets = NDT:GetRiftOffsets()
    ...
    self:SetScript("OnDragStart", function()
        self:StartMoving()
        NDT.draggedBlip = self
        local _,activeDoors = NDT.poi_framePools:GetPool("MapLinkPinTemplate"):EnumerateActive()
        riftOffsets = NDT:GetRiftOffsets()
        self:SetScript("OnUpdate",function()
            for poiFrame,_ in pairs(activeDoors) do
                if NDT:DoFramesOverlap(self,poiFrame,-10) then
                    poiFrame.HighlightTexture:Show()
                else
                    poiFrame.HighlightTexture:Hide()
                end
            ...
            if x ~= self.adjustedX or y~= self.adjustedY then
                local sizex,sizey = NDT:GetDefaultMapPanelSize()
                x = x<=0 and 0 or x>=sizex and sizex or x
                y = y>=0 and 0 or y<=(-1)*sizey and (-1)*sizey or y
                self.adjustedX = x
                self.adjustedY = y
                local connectedDoor = NDT:FindConnectedDoor(self.data.id)
                self.animatedLine = NDT:ShowAnimatedLine(NDT.main_frame.mapPanelFrame,connectedDoor or self.spireFrame ,self,nil,nil,nil,nil,nil,self.selected,self.animatedLine)
                riftOffsets[self.data.id] = riftOffsets[self.data.id] or {}
                riftOffsets[self.data.id].x = x
                riftOffsets[self.data.id].y = y
                self:ClearAllPoints()
                self:SetPoint("CENTER", NDT.main_frame.mapPanelTile1,"TOPLEFT",x*scale,y*scale)
            end
        end)
    end)
    self:SetScript("OnDragStop", function()
        ...
        self:SetScript("OnUpdate",nil)
        self:ClearAllPoints()
        self:SetPoint("CENTER", NDT.main_frame.mapPanelTile1,"TOPLEFT",self.adjustedX*scale,self.adjustedY*scale)
        --dragged ontop of door
        --find doors,check overlap,break,swap sublevel,change poi sublevel
        local _,active = NDT.poi_framePools:GetPool("MapLinkPinTemplate"):EnumerateActive()
        for poiFrame,_ in pairs(active) do
            if NDT:DoFramesOverlap(self,poiFrame,-10) then
                ...
                if shouldAdd then tinsert(c,{ connectionIndex=poiFrame.poi.connectionIndex, source= NDT:GetCurrentSubLevel()+0, target=poiFrame.poi.target}) end
        ...
        if NDT.liveSessionActive and NDT:GetCurrentPreset().uid == NDT.livePresetUID then
            NDT:LiveSession_SendCorruptedPositions(preset.value.riftOffsets)
        end
    end)
end

function NDTDungeonEnemyMixin:OnClick(button, down)

    if button == "LeftButton" then
        if IsShiftKeyDown() then
            local newPullIdx = NDT:GetCurrentPull() + 1
            NDT:PresetsAddPull(newPullIdx)
            NDT:GetCurrentPreset().value.selection = { newPullIdx}
            NDT:ReloadPullButtons()
            NDT:SetSelectionToPull(newPullIdx)
            local _
            NDT:ColorAllPulls(_, newPullIdx)
            --if NDT:GetPullsNum() == 2 then NDT:SetAutomaticColor(1) end
        end
        NDT:DungeonEnemies_AddOrRemoveBlipToCurrentPull(self,not self.selected,IsControlKeyDown())
        NDT:DungeonEnemies_UpdateSelected(NDT:GetCurrentPull())
        ...
        if self.data.corrupted then
            ...
            self.animatedLine = NDT:ShowAnimatedLine(NDT.main_frame.mapPanelFrame,connectedFrame,self,nil,nil,nil,nil,nil,self.selected,self.animatedLine)
    ...
    elseif button == "RightButton" then
        if db.devMode then
            if IsAltKeyDown() then
                tremove(NDT.dungeonEnemies[db.currentDungeonIdx][self.enemyIdx].clones,self.cloneIdx)
                self:Hide()
            else
...
function NDT:GetPatrolBlips()
    return patrolPoints
end

function NDTDungeonEnemyMixin:DisplayPatrol(shown)
    local scale = NDT:GetScale()
    ...
    if self.clone.patrol then
        ...
        for patrolIdx,waypoint in ipairs(self.clone.patrol) do
            patrolPoints[patrolIdx] = patrolPoints[patrolIdx] or NDT.main_frame.mapPanelFrame:CreateTexture("NDTDungeonPatrolPoint"..patrolIdx,"BACKGROUND")
            ...
            patrolPoints[patrolIdx]:SetPoint("CENTER", NDT.main_frame.mapPanelTile1,"TOPLEFT",waypoint.x*scale,waypoint.y*scale)
            patrolPoints[patrolIdx].x = waypoint.x
            patrolPoints[patrolIdx].y = waypoint.y
            patrolPoints[patrolIdx]:Show()

            patrolLines[patrolIdx] = patrolLines[patrolIdx] or NDT.main_frame.mapPanelFrame:CreateTexture("NDTDungeonPatrolLine"..patrolIdx,"BACKGROUND")
            patrolLines[patrolIdx]:SetDrawLayer("OVERLAY", 1)
            patrolLines[patrolIdx]:SetTexture("Interface\\AddOns\\NomadicDungeonTools\\Textures\\Square_White")
            ...
            if oldWaypointBlip then
                ...
                DrawLine(patrolLines[patrolIdx], NDT.main_frame.mapPanelTile1, startX, startY, endX, endY, 1*scale, 1,"TOPLEFT")
                patrolLines[patrolIdx]:Show()
            else
        ...
        if firstWaypointBlip and oldWaypointBlip then
            ...
            local endPoint, endRelativeTo, endRelativePoint, endX, endY = oldWaypointBlip:GetPoint()
            DrawLine(patrolLines[1], NDT.main_frame.mapPanelTile1, startX, startY, endX, endY, 1*scale, 1,"TOPLEFT")
            patrolLines[1]:Show()
        end
...
local ranOnce
function NDT:DisplayBlipTooltip(blip, shown)
    if not ranOnce then
        --fix elvui skinning
        NDT.tooltip:ClearAllPoints()
        NDT.tooltip:SetPoint("TOPLEFT",UIParent,"BOTTOMRIGHT")
        NDT.tooltip:SetPoint("BOTTOMRIGHT",UIParent,"BOTTOMRIGHT")
        NDT.tooltip:Show()
        NDT.tooltip:Hide()
        ranOnce = true
    end
    ...
    local health = NDT:CalculateEnemyHealth(boss,data.health,db.currentDifficulty,data.ignoreFortified)
    ...
    local text = data.name.." "..occurence..group.."\n"..string.format(L["Level %d %s"],data.level,data.creatureType).."\n".. string.format(L["%s HP"],NDT:FormatEnemyHealth(health)).."\n"
    local count = NDT:IsCurrentPresetTeeming() and data.teemingCount or data.count
    text = text ..L["Forces"]..": ".. NDT:FormatEnemyForces(count)
    local reapingText
    if blip.data.reaping and db.MDI.enabled and preset.mdi.beguiling == 13 then
        local reapingIcon = CreateTextureMarkup(NDT.reapingStatic[tostring(blip.data.reaping)].iconTexture, 32, 32, 16, 16, 0, 1, 0, 1,0,0) or ""
        reapingText = L["Reaping"]..": "..reapingIcon.." ".. NDT.reapingStatic[tostring(blip.data.reaping)].name .. "\n"
    end
    ...
    if db.tooltipInCorner then
        tooltip:SetPoint("BOTTOMRIGHT", NDT.main_frame,"BOTTOMRIGHT",0,0)
        tooltip:SetPoint("TOPLEFT", NDT.main_frame,"BOTTOMRIGHT",-tooltip.mySizes.x,tooltip.mySizes.y)
    else
        ...
        local mainFrameRight = NDT.main_frame:GetRight()
...
function NDT:GetCurrentDevmodeBlip()
    for blipIdx,blip in pairs(blips) do
        if blip.devSelected then
            return blip
        end
...
local function blipDevModeSetup(blip)
    ...
    blip:SetScript("OnMouseDown",function()
        ...
        x = x*(1/scale)
        y = y*(1/scale)
        local nx = NDT.dungeonEnemies[db.currentDungeonIdx][blip.enemyIdx].clones[blip.cloneIdx].x
        local ny = NDT.dungeonEnemies[db.currentDungeonIdx][blip.enemyIdx].clones[blip.cloneIdx].y
        xOffset = x-nx
        yOffset = y-ny
    end)
    ...
    blip:SetScript("OnDragStop", function()
        ...
        blip:ClearAllPoints()
        blip:SetPoint("CENTER", NDT.main_frame.mapPanelTile1,"TOPLEFT",x*scale,y*scale)
        NDT.dungeonEnemies[db.currentDungeonIdx][blip.enemyIdx].clones[blip.cloneIdx].x = x
        NDT.dungeonEnemies[db.currentDungeonIdx][blip.enemyIdx].clones[blip.cloneIdx].y = y
    end)
    local groupColors = {
        [1] = {1,0,0,1},
        [2] = {0,1,0,1},
    ...
    blip:SetScript("OnMouseWheel", function(self, delta)
        ...
        if not blip.clone.g then
            local maxGroup = 0
            for _,data in pairs(NDT.dungeonEnemies[db.currentDungeonIdx]) do
...
function NDTDungeonEnemyMixin:SetUp(data,clone)
    local scale = NDT:GetScale()
    self:ClearAllPoints()
    self:SetPoint("CENTER", NDT.main_frame.mapPanelTile1,"TOPLEFT",clone.x*scale,clone.y*scale)
    self.normalScale = data.scale*(data.isBoss and 1.7 or 1)*(NDT.scaleMultiplier[db.currentDungeonIdx] or 1)*scale
    ...
    if not data.corrupted then
        for k,v in pairs(blips) do
            ...
            if ((clone.x-v.clone.x)^2+(clone.y-v.clone.y)^2<81) and NDT:DoFramesOverlap(self, v,5) then raise = max(raise,v:GetFrameLevel()+1) end
        end
    end
    ...
    if data.corrupted then
        ...
        local riftOffsets = NDT:GetRiftOffsets()
...
function NDT:DungeonEnemies_IsAnyBlipMoving()
    local isAnyMoving
    for blipIdx,blip in pairs(blips) do
...
function NDT:DungeonEnemies_HideAllBlips()
    NDT.dungeonEnemies_framePools:ReleaseAll()
end

function NDT:DungeonEnemies_UpdateEnemies()
    NDT.dungeonEnemies_framePools:ReleaseAll()
    twipe(blips)
    if not db then db = NDT:GetDB() end
    local enemies = NDT.dungeonEnemies[db.currentDungeonIdx]
        ...
        for cloneIdx,clone in pairs(data["clones"]) do
            ...
            if clone.sublevel == currentSublevel or (not clone.sublevel) then
                ...
                if not (data.corrupted and riftOffsets and riftOffsets[data.id] and riftOffsets[data.id].sublevel) then
                    local blip = NDT.dungeonEnemies_framePools:Acquire("NDTDungeonEnemyTemplate")
...
function NDT:DungeonEnemies_CreateFramePools()
    db = self:GetDB()
    NDT.dungeonEnemies_framePools = NDT.dungeonEnemies_framePools or CreateFramePoolCollection()
    NDT.dungeonEnemies_framePools:CreatePool("Button", NDT.main_frame.mapPanelFrame, "NDTDungeonEnemyTemplate");
end

function NDT:FindPullOfBlip(blip)
...
function NDT:GetBlip(enemyIdx, cloneIdx, preset)
    for blipIdx,blip in pairs(blips) do
...
function NDT:DungeonEnemies_AddOrRemoveBlipToCurrentPull(blip, add, ignoreGrouped,pulls,pull,ignoreUpdates)
...
function NDT:DungeonEnemies_UpdateBlipColors(pull, r, g, b,pulls)
...
function NDT:DungeonEnemies_UpdateSelected(pull,pulls)
    preset = NDT:GetCurrentPreset()
    pulls = pulls or preset.value.pulls
    --deselect all
    for _,blip in pairs(blips) do
    ...
    for pullIdx,p in pairs(pulls) do
        local r,g,b = NDT:DungeonEnemies_GetPullColor(pullIdx)
        for enemyIdx,clones in pairs(p) do
            if tonumber(enemyIdx) then
                for _,cloneIdx in pairs(clones) do
                    for _,blip in pairs(blips) do
    ...
    NDT:DrawAllHulls(pulls)
end

---DungeonEnemies_SetPullColor
---Sets a custom color for a pull
function NDT:DungeonEnemies_SetPullColor(pull, r, g, b)
    preset = NDT:GetCurrentPreset()
    preset.value.pulls[pull]["color"] = NDT:RGBToHex(r,g,b)
end

---DungeonEnemies_GetPullColor
---Returns the custom color for a pull
function NDT:DungeonEnemies_GetPullColor(pull,pulls)
    pulls = pulls or preset.value.pulls
    local r,g,b = NDT:HexToRGB(pulls[pull]["color"])
    if not r then
        r,g,b = NDT:HexToRGB(db.defaultColor)
        NDT:DungeonEnemies_SetPullColor(pull,r,g,b)
    end
    return r,g,b
end
...
function NDT:DungeonEnemies_UpdateTeeming()
    preset = NDT:GetCurrentPreset()
...
function NDT:DungeonEnemies_UpdateSeasonalAffix()
...
function NDT:DungeonEnemies_UpdateBlacktoothEvent()
    local week
...
function NDT:DungeonEnemies_UpdateBoralusFaction(faction)
    preset = NDT:GetCurrentPreset()
    local teeming = NDT:IsPresetTeeming(preset)
    for _,blip in pairs(blips) do
...
function NDT:DungeonEnemies_UpdateReaping()
    for _,blip in pairs(blips) do
...
function NDT:DungeonEnemies_UpdateInfested(week)
    week = preset.week%3
    if week == 0 then week = 3 end
    for _,blip in pairs(blips) do
...
function NDT:DungeonEnemies_UpdateInspiring(week)
    week = week or preset.week
    local isInspiring = NDT:IsWeekInspiring(week)
    for _,blip in pairs(blips) do
...
function NDT:DungeonEnemies_UpdateFreeholdCrew(crewIdx)
    ...
    local crew = NDT.freeholdCrews[crewIdx]
    for _,blip in pairs(blips) do
        ...
        if enemy.id == poi.npcId then
            ...
            for cloneIdx,clone in pairs(enemy.clones) do
                if clone.week[week] then
                    return NDT:IsCloneInPulls(enemyIdx,cloneIdx)
                end
            end
        end
    end
...
function NDT:IsCloneInPulls(enemyIdx, cloneIdx)
    local pulls = NDT:GetCurrentPreset().value.pulls
    local numClones = 0
...
function NDT:GetEnemyForces(npcId)
...
function NDT:GetNPCNameById(npcId)
    local data = NDT.dungeonEnemies[db.currentDungeonIdx]
    if data then
...
function NDT:UpdateDungeonData(dungeonData)

    local function printDungeonName(shouldPrint,dungeonIdx)
        if shouldPrint then
            print("-----", NDT:GetDungeonName(dungeonIdx))
        end
        return false
    end
    ...
    for dungeonIdx,newData in pairs(dungeonData) do
        ...
        local totalCount = NDT.dungeonTotalCount[dungeonIdx]
        ...
        local enemyData = NDT.dungeonEnemies[dungeonIdx]
        if enemyData then
...
--]
function NDT:ExportNPCIdsWithoutDisplayIds()
    local output = "dungeons = [\n"
    for idx = 15, NDT:GetNumDungeons() do
        local shouldAddDungeonText = true
        local enemyData = NDT.dungeonEnemies[idx]
        if enemyData then
            for _,enemy in pairs(enemyData) do
                if not enemy.displayId then
                    if shouldAddDungeonText then
                        output = output.."Dungeon(name='".. NDT:GetDungeonName(idx).."', idx=".. idx..", npcIds=["
    ...
    NDT.main_frame.ExportFrameEditbox:SetLabel("NPC ids without displayId")
end
...
function NDT:CleanEnemyData(dungeonIdx)
    local enemies = NDT.dungeonEnemies[dungeonIdx]
