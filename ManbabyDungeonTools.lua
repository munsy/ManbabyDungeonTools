...
NDT.BackdropColor = { 0.058823399245739, 0.058823399245739, 0.058823399245739, 0.9}
...
local LDB = LibStub("LibDataBroker-1.1"):NewDataObject("ManbabyDungeonTools", {
	...
	OnTooltipShow = function(tooltip)
		...
		tooltip:AddLine(mythicColor .."Nomadic Dungeon Tools|r")
...
function SlashCmdList.MANBABYDUNGEONTOOLS(cmd, editbox)
	...
	elseif rqst == "reset" then
        NDT:ResetMainFramePos()
	elseif rqst == "dc" then
        NDT:ToggleDataCollection()
    elseif rqst == "hptrack" then
        NDT:ToggleHealthTrack()
    else
		NDT:ShowInterface()
	end
end

function NDT:GetLocaleIndex()
    local localeToIndex = {
        ["enUS"] = 1,
        ["deDE"] = 2,
        ["esES"] = 3,
...
do
    ...
    local last = 0
    function NDT.GROUP_ROSTER_UPDATE(self, addon)
        --check not more than once per second (blizzard event spam)
        local now = GetTime()
        if last < now - 1 then
            if not NDT.main_frame then return end
            local inGroup = UnitInRaid("player") or IsInGroup()
            NDT.main_frame.LinkToChatButton:SetDisabled(not inGroup)
            NDT.main_frame.LiveSessionButton:SetDisabled(not inGroup)
            if inGroup then
                ...
                if NDT.liveSessionActive then
                    ...
                    NDT.main_frame.LiveSessionButton.text:SetTextColor(0,1,0)
                else
                    NDT.main_frame.LiveSessionButton:SetText(L["Live"])
                    NDT.main_frame.LiveSessionButton.text:SetTextColor(1,0.8196,0)
                end
            else
                NDT.main_frame.LinkToChatButton.text:SetTextColor(0.5,0.5,0.5)
                NDT.main_frame.LiveSessionButton.text:SetTextColor(0.5,0.5,0.5)
            end
            last = now
            --NDT:RequestDataCollectionUpdate()
        end
    end
    function NDT.PLAYER_ENTERING_WORLD(self, addon)
...
function NDT:GetNumDungeons() return #dungeonList-1 end
function NDT:GetDungeonName(idx) return dungeonList[idx] end
...
function NDT:GetDungeonSublevels()
    return dungeonSubLevels
end

function NDT:GetSublevelName(dungeonIdx, sublevelIdx)
...
function NDT:ShowInterface(force)
	...
	if self.main_frame:IsShown() and not force then
        ...
        --edge case if user closed NDT window while in the process of dragging a corrupted blip
        if self.draggedBlip then
            if NDT.liveSessionActive then
                NDT:LiveSession_SendCorruptedPositions(NDT:GetRiftOffsets())
            end
            self:UpdateMap()
            self.draggedBlip = nil
        end
        NDT:UpdateBottomText()
	end
end

function NDT:HideInterface()
	self.main_frame:Hide()
	self.main_frame.HelpButton:Hide()
end

function NDT:ToggleDevMode()
    db.devMode = not db.devMode
    ReloadUI()
end

function NDT:ToggleDataCollection()
    db.dataCollectionActive = not db.dataCollectionActive
    print(string.format("%sNDT|r: DataCollection %s. Reload Interface!", mythicColor,db.dataCollectionActive and "|cFF00FF00Enabled|r" or "|cFFFF0000Disabled|r"))
end

function NDT:ToggleHealthTrack()
    NDT.DataCollection:InitHealthTrack()
    print(string.format("%sNDT|r: HealthTrack %s.", mythicColor,"|cFF00FF00Enabled|r"))
end


function NDT:CreateMenu()
    -- Close button
    self.main_frame.closeButton = CreateFrame("Button", "NDTCloseButton", self.main_frame, "UIPanelCloseButton")
    ...
    --Maximize Button
    self.main_frame.maximizeButton = CreateFrame("Button", "NDTMaximizeButton", self.main_frame, "MaximizeMinimizeButtonFrameTemplate")
    ...
    self.main_frame.liveReturnButton = CreateFrame("Button", "NDTLiveReturnButton", self.main_frame, "UIPanelCloseButton")
    ...
    self.main_frame.setLivePresetButton = CreateFrame("Button", "NDTSetLivePresetButton", self.main_frame, "UIPanelCloseButton")
...
function NDT:GetDefaultMapPanelSize()
    return sizex,sizey
end

---GetScale
---Returns scale factor stored in db
function NDT:GetScale()
...
function NDT:GetFullScreenSizes()
...
---Maximize
---FULLSCREEN the UI
function NDT:Maximize()
    local f = NDT.main_frame
    ...
    if not f.blackoutFrame then
        f.blackoutFrame = CreateFrame("Frame", "NDTBlackoutFrame", f)
    ...
    local newSizex, newSizey, scale, isNarrow = NDT:GetFullScreenSizes()
    db.scale = scale
    f:ClearAllPoints()
    ...
    f.resizer:Hide()
    NDT:CreateTutorialButton(NDT.main_frame)
    db.maximized = true
end

---Minimize
---Restore normal UI
function NDT:Minimize()
    local f = NDT.main_frame
...
function NDT:SkinProgressBar(progressBar)
...
function NDT:IsFrameOffScreen()
    local topPanel = NDT.main_frame.topPanel
    local bottomPanel = NDT.main_frame.bottomPanel
...
local bottomTips = {
    ...
    [7] = L["Drag the bottom right edge to resize NDT."],
    [8] = L["Click the fullscreen button for a maximized view of NDT."],
    [9] = L["Use /Ndt reset to restore the default position and scale of NDT."],
    [10] = L["Mouseover the Live button while in a group to learn more about Live mode."],
    [11] = L["You are using NDT. You rock!"],
...
function NDT:UpdateBottomText()
    local f = self.main_frame.bottomPanelString
    f:SetText(bottomTips[math.random(#bottomTips)])
end

function NDT:MakeTopBottomTextures(frame)
    frame:SetMovable(true)
	if frame.topPanel == nil then
		frame.topPanel = CreateFrame("Frame", "NDTTopPanel", frame)
		frame.topPanelTex = frame.topPanel:CreateTexture(nil, "BACKGROUND")
		frame.topPanelTex:SetAllPoints()
		frame.topPanelTex:SetDrawLayer(canvasDrawLayer, -5)
		frame.topPanelTex:SetColorTexture(unpack(NDT.BackdropColor))
		frame.topPanelString = frame.topPanel:CreateFontString("NDT name")
    ...
    frame.topPanel:SetScript("OnDragStop", function(self,button)
        ...
        if NDT:IsFrameOffScreen() then
            NDT:ResetMainFramePos(true)
        else
            local from,_,to,x,y = NDT.main_frame:GetPoint()
            db.anchorFrom = from
            db.anchorTo = to
    ...
    if frame.bottomPanel == nil then
        frame.bottomPanel = CreateFrame("Frame", "NDTBottomPanel", frame)
        ...
        frame.bottomPanelTex:SetColorTexture(unpack(NDT.BackdropColor))
    end
    ...
    frame.bottomPanelString = frame.bottomPanel:CreateFontString("NDTMid")
    ...
    frame.bottomLeftPanelString = frame.bottomPanel:CreateFontString("NDTVersion")
...
function NDT:MakeSidePanel(frame)

	if frame.sidePanel == nil then
		frame.sidePanel = CreateFrame("Frame", "NDTSidePanel", frame)
		...
		frame.sidePanelTex:SetColorTexture(unpack(NDT.BackdropColor))
	...
	frame.sidePanelString = frame.sidePanel:CreateFontString("NDTSidePanelText")
	...
	dropdown:SetCallback("OnValueChanged",function(widget,callbackName,key)
		if db.presets[db.currentDungeonIdx][key].value==0 then
			NDT:OpenNewPresetDialog()
			NDT.main_frame.sidePanelDeleteButton:SetDisabled(true)
			NDT.main_frame.sidePanelDeleteButton.text:SetTextColor(0.5,0.5,0.5)
		else
			if key == 1 then
				NDT.main_frame.sidePanelDeleteButton:SetDisabled(true)
                NDT.main_frame.sidePanelDeleteButton.text:SetTextColor(0.5,0.5,0.5)
			else
                if not NDT.liveSessionActive then
                    NDT.main_frame.sidePanelDeleteButton:SetDisabled(false)
                    NDT.main_frame.sidePanelDeleteButton.text:SetTextColor(1,0.8196,0)
                else
                    NDT.main_frame.sidePanelDeleteButton:SetDisabled(true)
                    NDT.main_frame.sidePanelDeleteButton.text:SetTextColor(0.5,0.5,0.5)
                end
            ...
            --frame.sidePanel.affixDropdown:SetAffixWeek(NDT:GetCurrentPreset().week or NDT:GetCurrentAffixWeek())
			--UpdateMap is called in SetAffixWeek, no need to call twice
            NDT:UpdateMap()
            frame.sidePanel.affixDropdown:SetAffixWeek(NDT:GetCurrentPreset().week or NDT:GetCurrentAffixWeek() or 1)
		end
	end)
	NDT:UpdatePresetDropDown()
	...
	--button fontInstance
	local fontInstance = CreateFont("NDTButtonFont")
	...
	frame.sidePanelRenameButton:SetCallback("OnClick",function(widget,callbackName,value)
		...
		NDT.main_frame.RenameFrame:Show()
		NDT.main_frame.RenameFrame.RenameButton:SetDisabled(true)
		NDT.main_frame.RenameFrame.RenameButton.text:SetTextColor(0.5,0.5,0.5)
        NDT.main_frame.RenameFrame:ClearAllPoints()
		NDT.main_frame.RenameFrame:SetPoint("CENTER", NDT.main_frame,"CENTER",0,50)
		NDT.main_frame.RenameFrame.Editbox:SetText(currentPresetName)
		NDT.main_frame.RenameFrame.Editbox:HighlightText(0, string.len(currentPresetName))
		NDT.main_frame.RenameFrame.Editbox:SetFocus()
	end)
    ...
    frame.sidePanelImportButton.frame:SetScript("OnEnter",function()
        ...
        GameTooltip:AddLine(L["You can find NDT exports from other users on the wago.io website"],1,1,1,1)
        GameTooltip:Show()
    end)
	...
	frame.sidePanelExportButton:SetCallback("OnClick",function(widget,callbackName,value)
        if db.colorPaletteInfo.forceColorBlindMode then NDT:ColorAllPulls(_,_,_,true) end
        local preset = NDT:GetCurrentPreset()
        NDT:SetUniqueID(preset)
        preset.mdiEnabled = db.MDI.enabled
        preset.difficulty = db.currentDifficulty
		local export = NDT:TableToString(preset,true,5)
		NDT:HideAllDialogs()
		NDT.main_frame.ExportFrame:Show()
        NDT.main_frame.ExportFrame:ClearAllPoints()
		NDT.main_frame.ExportFrame:SetPoint("CENTER", NDT.main_frame,"CENTER",0,50)
		NDT.main_frame.ExportFrameEditbox:SetText(export)
		NDT.main_frame.ExportFrameEditbox:HighlightText(0, string.len(export))
		NDT.main_frame.ExportFrameEditbox:SetFocus()
        NDT.main_frame.ExportFrameEditbox:SetLabel(preset.text.." "..string.len(export))
        if db.colorPaletteInfo.forceColorBlindMode then NDT:ColorAllPulls() end
    end)
    frame.sidePanelExportButton.frame:SetScript("OnEnter",function()
        ...
        GameTooltip:AddLine(L["You can share NDT exports on the wago.io website"],1,1,1,1)
        GameTooltip:Show()
    end)
	...
	frame.sidePanelDeleteButton:SetCallback("OnClick",function(widget,callbackName,value)
        if IsShiftKeyDown() then
            ...
            NDT:OpenConfirmationFrame(450,150,L["Delete ALL presets"],L["Delete"],prompt, NDT.DeleteAllPresets)
        else
            NDT:HideAllDialogs()
            frame.DeleteConfirmationFrame:ClearAllPoints()
            frame.DeleteConfirmationFrame:SetPoint("CENTER", NDT.main_frame,"CENTER",0,50)
            local currentPresetName = db.presets[db.currentDungeonIdx][db.currentPreset[db.currentDungeonIdx]].text
	...
	frame.LinkToChatButton:SetCallback("OnClick",function(widget,callbackName,value)
        local distribution = NDT:IsPlayerInGroup()
        ...
        if presetSize>25000 then
            ...
            NDT:OpenConfirmationFrame(450,150,L["Sharing large preset"],"Share",prompt, callback)
        else
            callback()
        end
	end)
    ...
    local function makeAffixString(week,affixes,longText)
        ...
        for _,affixID in ipairs(affixes) do
            ...
            if longText then
                ...
                if week == NDT:GetCurrentAffixWeek() then
    ...
    function affixDropdown:SetAffixWeek(key,ignoreReloadPullButtons,ignoreUpdateProgressBar)
        ...
        elseif NDT:GetCurrentAffixWeek() == key then
            frame.sidePanel.affixWeekWarning.image:Hide()
            frame.sidePanel.affixWeekWarning:SetDisabled(true)
        else
        ...
        NDT:GetCurrentPreset().week = key
        local teeming = NDT:IsPresetTeeming(NDT:GetCurrentPreset())
        NDT:GetCurrentPreset().value.teeming = teeming

        if NDT.EnemyInfoFrame and NDT.EnemyInfoFrame.frame:IsShown() then NDT:UpdateEnemyInfoData() end
        NDT:UpdateMap(nil,ignoreReloadPullButtons,ignoreUpdateProgressBar)
    end
    affixDropdown:SetCallback("OnValueChanged",function(widget,callbackName,key)
        ...
        if NDT.liveSessionActive and NDT:GetCurrentPreset().uid == NDT.livePresetUID then
            NDT:LiveSession_SendAffixWeek(key)
        end
    end)
    ...
    affixWeekWarning:SetCallback("OnClick",function(...)
        if not NDT:GetCurrentAffixWeek() then return end
        affixDropdown:SetAffixWeek(NDT:GetCurrentAffixWeek())
        if NDT.liveSessionActive and NDT:GetCurrentPreset().uid == NDT.livePresetUID then
            NDT:LiveSession_SendAffixWeek(NDT:GetCurrentAffixWeek())
        end
    end)
	...
	frame.sidePanel.DifficultySlider:SetCallback("OnValueChanged",function(widget,callbackName,value)
        ...
        NDT:GetCurrentPreset().difficulty = db.currentDifficulty
        ...
        timer = C_Timer.NewTimer(2, function()
            ...
            if NDT.liveSessionActive then
                local livePreset = NDT:GetCurrentLivePreset()
                local shouldUpdate = livePreset == NDT:GetCurrentPreset()
                if shouldUpdate then NDT:LiveSession_SendDifficulty() end
            end
        end)
	end)
    ...
    NDT:SkinProgressBar(frame.sidePanel.ProgressBar)
end

---ToggleMDIMode
---Enables display to override beguiling+freehold week
function NDT:ToggleMDIMode()
...
function NDT:DisplayMDISelector()
    local show = db.MDI.enabled
    db = NDT:GetDB()
    if not NDT.MDISelector then
        NDT.MDISelector = AceGUI:Create("SimpleGroup")
        NDT.MDISelector.frame:SetFrameStrata("HIGH")
        NDT.MDISelector.frame:SetFrameLevel(50)
        if not NDT.MDISelector.frame.SetBackdrop then
            Mixin(NDT.MDISelector.frame, BackdropTemplateMixin)
        end
        NDT.MDISelector.frame:SetBackdropColor(unpack(NDT.BackdropColor))
        --fix show hide
        local frame = NDT.main_frame
        local originalShow,originalHide = frame.Show,frame.Hide
        local widget = NDT.MDISelector.frame
        function frame:Hide(...)
            widget:Hide()
            return originalHide(self, ...)
        end
        ...
        NDT.MDISelector:SetLayout("Flow")
        NDT.MDISelector.frame.bg = NDT.MDISelector.frame:CreateTexture(nil, "BACKGROUND")
        NDT.MDISelector.frame.bg:SetAllPoints(NDT.MDISelector.frame)
        NDT.MDISelector.frame.bg:SetColorTexture(unpack(NDT.BackdropColor))
        NDT.MDISelector:SetWidth(145)
        NDT.MDISelector:SetHeight(90)
        NDT.MDISelector.frame:ClearAllPoints()
        NDT.MDISelector.frame:SetPoint("BOTTOMRIGHT", NDT.main_frame,"BOTTOMRIGHT",0,0)

        local label = AceGUI:Create("Label")
        label:SetText(L["MDI Mode"])
        NDT.MDISelector:AddChild(label)

        --beguiling
        NDT.MDISelector.BeguilingDropDown = AceGUI:Create("Dropdown")
        NDT.MDISelector.BeguilingDropDown:SetLabel(L["Seasonal Affix:"])
        ...
        NDT.MDISelector.BeguilingDropDown:SetList(beguilingList)
        NDT.MDISelector.BeguilingDropDown:SetCallback("OnValueChanged",function(widget, callbackName, key)
        ...
        NDT.MDISelector:AddChild(NDT.MDISelector.BeguilingDropDown)

        --freehold
        NDT.MDISelector.FreeholdDropDown = AceGUI:Create("Dropdown")
        NDT.MDISelector.FreeholdDropDown:SetLabel(L["Freehold:"])
        local freeholdList = {string.format("1. %s",L["Cutwater"]),string.format("2. %s",L["Blacktooth"]),string.format("3. %s",L["Bilge Rats"])}
        NDT.MDISelector.FreeholdDropDown:SetList(freeholdList)
        NDT.MDISelector.FreeholdDropDown:SetCallback("OnValueChanged",function(widget, callbackName, key)
            local preset = NDT:GetCurrentPreset()
        ...
        NDT.MDISelector:AddChild(NDT.MDISelector.FreeholdDropDown)

        NDT.MDISelector.FreeholdCheck = AceGUI:Create("CheckBox")
        NDT.MDISelector.FreeholdCheck:SetLabel(L["Join Crew"])
        NDT.MDISelector.FreeholdCheck:SetCallback("OnValueChanged",function(widget, callbackName, value)
            local preset = NDT:GetCurrentPreset()
        ...
        NDT.MDISelector:AddChild(NDT.MDISelector.FreeholdCheck)
    ...
    if show then
        ...
        db.currentSeason = NDT:GetEffectivePresetSeason(preset)
        NDT:DungeonEnemies_UpdateSeasonalAffix()
        ...
        NDT.MDISelector.FreeholdCheck:SetValue(preset.mdi.freeholdJoined)
        NDT:DungeonEnemies_UpdateFreeholdCrew()
        NDT:DungeonEnemies_UpdateBlacktoothEvent()
        NDT:UpdateProgressbar()
        NDT:ReloadPullButtons()
        NDT.MDISelector.frame:Show()
        NDT:ToggleFreeholdSelector(false)
    else
        ...
        NDT:UpdateProgressbar()
        NDT:ReloadPullButtons()
        NDT.MDISelector.frame:Hide()
        NDT:ToggleFreeholdSelector(db.currentDungeonIdx == 16)
    end
    NDT:POI_UpdateAll()
    NDT:KillAllAnimatedLines()
    NDT:DrawAllAnimatedLines()
...
function NDT:UpdatePresetDropDown()
	local dropdown = NDT.main_frame.sidePanel.WidgetGroup.PresetDropDown
	local presetList = {}
...
function NDT:UpdatePresetDropdownTextColor(forceReset)
    local preset = self:GetCurrentPreset()
...
---FormatEnemyForces
function NDT:FormatEnemyForces(forces, forcesmax, progressbar)
    if not forcesmax then forcesmax = NDT:IsCurrentPresetTeeming() and NDT.dungeonTotalCount[db.currentDungeonIdx].teeming or NDT.dungeonTotalCount[db.currentDungeonIdx].normal end
...
function NDT:Progressbar_SetValue(self, totalCurrent, totalMax)
	...
	self.Bar.Label:SetText(NDT:FormatEnemyForces(totalCurrent,totalMax,true))
	self.AnimValue = percent
end

---UpdateProgressbar
---Update the progressbar on the sidepanel with the correct values
function NDT:UpdateProgressbar()
	local teeming = db.presets[db.currentDungeonIdx][db.currentPreset[db.currentDungeonIdx]].value.teeming
    NDT:EnsureDBTables()
    local grandTotal = NDT:CountForces()
	NDT:Progressbar_SetValue(NDT.main_frame.sidePanel.ProgressBar,grandTotal,teeming==true and NDT.dungeonTotalCount[db.currentDungeonIdx].teeming or NDT.dungeonTotalCount[db.currentDungeonIdx].normal)
end

function NDT:OnPan(cursorX, cursorY)
    local scrollFrame = NDTScrollFrame
    local scale = NDTMapPanelFrame:GetScale()/1.5
...
function NDT:OnPanFadeOut(deltaTime)
    local scrollFrame = NDTScrollFrame
...
function NDT:ExportCurrentZoomPanSettings()
    local mainFrame = NDTMapPanelFrame
    local scrollFrame = NDTScrollFrame

    local zoom = NDTMapPanelFrame:GetScale()
    local panH = NDTScrollFrame:GetHorizontalScroll() / NDT:GetScale()
    local panV = NDTScrollFrame:GetVerticalScroll() / NDT:GetScale()
    ...
    NDT.main_frame.ExportFrame:ClearAllPoints()
    NDT.main_frame.ExportFrame:SetPoint("CENTER", NDT.main_frame,"CENTER",0,50)
    NDT.main_frame.ExportFrameEditbox:SetText(output)
    NDT.main_frame.ExportFrameEditbox:HighlightText(0, string.len(output))
    NDT.main_frame.ExportFrameEditbox:SetFocus()
    NDT.main_frame.ExportFrameEditbox:SetLabel("Current pan/zoom settings");
end


function NDT:ZoomMapToDefault()
    ...
    local currentMapInfo = NDT.mapInfo[db.currentDungeonIdx]
    if(currentMapInfo and currentMapInfo.viewportPositionOverrides and currentMapInfo.viewportPositionOverrides[currentSublevel])then
        ...
        scrollFrame:SetHorizontalScroll(data.horizontalPan * NDT:GetScale())
        scrollFrame:SetVerticalScroll(data.verticalPan * NDT:GetScale())
...
function NDT:ZoomMap(delta)
    ...
    NDT:SetPingOffsets(newScale)
end

---ActivatePullTooltip
---
function NDT:ActivatePullTooltip(pull)
    local pullTooltip = NDT.pullTooltip
    --[[
...
function NDT:UpdatePullTooltip(tooltip)
    local frame = NDT.main_frame
    ...
    elseif MouseIsOver(frame.sidePanel.newPullButton.frame) then
		...
		if frame.sidePanel.newPullButtons and tooltip.currentPull and frame.sidePanel.newPullButtons[tooltip.currentPull] then
			...
			for k,v in pairs(frame.sidePanel.newPullButtons[tooltip.currentPull].enemyPortraits) do
				if MouseIsOver(v) then
					if v:IsShown() then
                        ...
                        local health = NDT:CalculateEnemyHealth(boss,v.enemyData.baseHealth,db.currentDifficulty,v.enemyData.ignoreFortified)
                        text = text.. string.format(L["%s HP"],NDT:FormatEnemyHealth(health))..newLine

                        local totalForcesMax = NDT:IsCurrentPresetTeeming() and NDT.dungeonTotalCount[db.currentDungeonIdx].teeming or NDT.dungeonTotalCount[db.currentDungeonIdx].normal
                        local count = NDT:IsCurrentPresetTeeming() and v.enemyData.teemingCount or v.enemyData.count
                        text = text..L["Forces"]..": ".. NDT:FormatEnemyForces(count,totalForcesMax,false)
            ...
            local pullForces = NDT:CountForces(tooltip.currentPull,true)
            local totalForces = NDT:CountForces(tooltip.currentPull,false)
            local totalForcesMax = NDT:IsCurrentPresetTeeming() and NDT.dungeonTotalCount[db.currentDungeonIdx].teeming or NDT.dungeonTotalCount[db.currentDungeonIdx].normal

            local text = L["Forces"]..": ".. NDT:FormatEnemyForces(pullForces,totalForcesMax,false)
            text = text.. "\n"..L["Total"]..": ".. NDT:FormatEnemyForces(totalForces,totalForcesMax,true)
...
function NDT:CountForces(currentPull, currentOnly)
                ...
                for enemyIdx,clones in pairs(pull) do
                    if tonumber(enemyIdx) then
                        for k,v in pairs(clones) do
                            if NDT:IsCloneIncluded(enemyIdx,v) then
                                local count = teeming
...
function NDT:IsCloneIncluded(enemyIdx, cloneIdx)
    local preset = NDT:GetCurrentPreset()
    local isCloneBlacktoothEvent = NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["clones"][cloneIdx].blacktoothEvent
    local cloneFaction = NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["clones"][cloneIdx].faction

    local week = self:GetEffectivePresetWeek()

    if db.currentSeason ~= 3 then
        if emissaryIds[NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx].id] then return false end
    elseif db.currentSeason ~= 4 then
        if NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx].corrupted then return false end
    end

    --beguiling weekly configuration
    local weekData = NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["clones"][cloneIdx].week
    if weekData then
    ...
    if not isCloneBlacktoothEvent or isBlacktoothWeek then
        if not (cloneFaction and cloneFaction~= preset.faction) then
            local isCloneTeeming = NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["clones"][cloneIdx].teeming
            local isCloneNegativeTeeming = NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["clones"][cloneIdx].negativeTeeming
            if NDT:IsCurrentPresetTeeming() or ((isCloneTeeming and isCloneTeeming == false) or (not isCloneTeeming)) then
                if not(NDT:IsCurrentPresetTeeming() and isCloneNegativeTeeming) then
                    return true
                end
            end
        end
...
function NDT:IsCurrentPresetTeeming()
...
function NDT:IsCurrentPresetFortified()
    return self:GetCurrentPreset().week%2 == 1
end

---IsCurrentPresetTyrannical
function NDT:IsCurrentPresetTyrannical()
    return not NDT:IsCurrentPresetFortified()
end

---MouseDownHook
function NDT:MouseDownHook()
    return
end

---Handles mouse-down events on the map scrollframe
NDT.OnMouseDown = function(self, button)
...
NDT.OnMouseUp = function(self, button)
	local scrollFrame = NDT.main_frame.scrollFrame
...
function NDT:SetPingOffsets(mapScale)
    local scale = 0.35
    local offset = (10.25/1000)*mapScale
    NDT.ping:SetTransform(offset,offset,0,0,0,0,scale)
end

---SetCurrentSubLevel
---Sets the sublevel of the currently active preset, need to UpdateMap to reflect the change in UI
function NDT:SetCurrentSubLevel(sublevel)
    NDT:GetCurrentPreset().value.currentSublevel = sublevel
end

---GetCurrentPull
---Returns the current pull of the currently active preset
function NDT:GetCurrentPull()
...
function NDT:GetCurrentSubLevel()
	return NDT:GetCurrentPreset().value.currentSublevel
end

---GetCurrentPreset
---Returns the current preset
function NDT:GetCurrentPreset()
    return db.presets[db.currentDungeonIdx][db.currentPreset[db.currentDungeonIdx]]
end

---GetCurrentLivePreset
function NDT:GetCurrentLivePreset()
...
function NDT:GetEffectivePresetWeek(preset)
...
function NDT:GetEffectivePresetSeason(preset)
...
---ReturnToLivePreset
function NDT:ReturnToLivePreset()
...
function NDT:IsWeekTeeming(week)
    if not week then week = NDT:GetCurrentAffixWeek() or 1 end
    return affixWeeks[week][1] == 5
end

---Returns if the current week has an affix weeks set that includes the inspiring affix
function NDT:IsWeekInspiring(week)
    if not week then week = NDT:GetCurrentAffixWeek() or 1 end
...
function NDT:IsPresetTeeming(preset)
    return NDT:IsWeekTeeming(preset.week)
end

function NDT:GetRiftOffsets()
    local week = self:GetEffectivePresetWeek()
    local preset = self:GetCurrentPreset()
...
function NDT:MakeMapTexture(frame)
    NDT.contextMenuList = {}

    tinsert(NDT.contextMenuList, {
        text = "Close",
        notCheckable = 1,
	...
	if frame.scrollFrame == nil then
		frame.scrollFrame = CreateFrame("ScrollFrame", "NDTScrollFrame",frame)
		...
		frame.scrollFrame:SetScript("OnMouseWheel", function(self, delta)
            if IsControlKeyDown() then
                if not lastModifiedScroll or lastModifiedScroll < GetTime() - 0.1 then
                    ...
                    delta = delta*-1
                    local target = NDT:GetCurrentSubLevel()+delta
                    if dungeonSubLevels[db.currentDungeonIdx][target] then
                        NDT:SetCurrentSubLevel(target)
                        NDT:UpdateMap()
                        NDT:ZoomMapToDefault()
                    end
                end
            elseif IsAltKeyDown() then
                if not lastModifiedScroll or lastModifiedScroll < GetTime() - 0.3 then
                    ...
                    if dungeonList[target] and not ignoredTargets[target] then
                        local group = NDT.main_frame.DungeonSelectionGroup
		...
		frame.scrollFrame:SetScript("OnUpdate", function(self,elapsed)
			...
			NDT:OnPan(x, y)
            NDT:OnPanFadeOut(elapsed)
        end)

		if frame.mapPanelFrame == nil then
			frame.mapPanelFrame = CreateFrame("frame","NDTMapPanelFrame",nil)
		...
		for i=1,12 do
			frame["mapPanelTile"..i] = frame.mapPanelFrame:CreateTexture("NDTmapPanelTile"..i, "BACKGROUND")
        ...
        for i=1,10 do
            for j=1,15 do
                frame["largeMapPanelTile"..i..j] = frame.mapPanelFrame:CreateTexture("NDTLargeMapPanelTile"..i..j, "BACKGROUND")
...
function NDT:CalculateEnemyHealth(boss, baseHealth, level, ignoreFortified)
    local fortified = NDT:IsCurrentPresetFortified()
    local tyrannical = NDT:IsCurrentPresetTyrannical()
	local mult = 1
...
function NDT:ReverseCalcEnemyHealth(unit, level, boss)
...
function NDT:FormatEnemyHealth(amount)
...
function NDT:UpdateDungeonEnemies()
    NDT:DungeonEnemies_UpdateEnemies()
end

function NDT:HideAllDialogs()
	NDT.main_frame.presetCreationFrame:Hide()
	NDT.main_frame.presetImportFrame:Hide()
	NDT.main_frame.ExportFrame:Hide()
	NDT.main_frame.RenameFrame:Hide()
	NDT.main_frame.ClearConfirmationFrame:Hide()
	NDT.main_frame.DeleteConfirmationFrame:Hide()
    NDT.main_frame.automaticColorsFrame.CustomColorFrame:Hide()
    NDT.main_frame.automaticColorsFrame:Hide()
    if NDT.main_frame.ConfirmationFrame then NDT.main_frame.ConfirmationFrame:Hide() end
end

function NDT:OpenImportPresetDialog()
	NDT:HideAllDialogs()
    NDT.main_frame.presetImportFrame:ClearAllPoints()
	NDT.main_frame.presetImportFrame:SetPoint("CENTER", NDT.main_frame,"CENTER",0,50)
	NDT.main_frame.presetImportFrame:Show()
	NDT.main_frame.presetImportBox:SetText("")
	NDT.main_frame.presetImportBox:SetFocus()
    NDT.main_frame.presetImportLabel:SetText(nil)
end

function NDT:OpenNewPresetDialog()
	NDT:HideAllDialogs()
	local presetList = {}
	...
	NDT.main_frame.PresetCreationDropDown:SetList(presetList)
	NDT.main_frame.PresetCreationDropDown:SetValue(1)
	NDT.main_frame.PresetCreationEditbox:SetText(L["defaultPresetName"].." "..countPresets+1)
    NDT.main_frame.presetCreationFrame:ClearAllPoints()
	NDT.main_frame.presetCreationFrame:SetPoint("CENTER", NDT.main_frame,"CENTER",0,50)
	NDT.main_frame.presetCreationFrame:SetStatusText("")
	NDT.main_frame.presetCreationFrame:Show()
	NDT.main_frame.presetCreationCreateButton:SetDisabled(false)
	NDT.main_frame.presetCreationCreateButton.text:SetTextColor(1,0.8196,0)
	NDT.main_frame.PresetCreationEditbox:SetFocus()
	NDT.main_frame.PresetCreationEditbox:HighlightText(0,50)
	NDT.main_frame.presetImportBox:SetText("")
end

function NDT:OpenClearPresetDialog()
    NDT:HideAllDialogs()
    NDT.main_frame.ClearConfirmationFrame:ClearAllPoints()
    NDT.main_frame.ClearConfirmationFrame:SetPoint("CENTER", NDT.main_frame,"CENTER",0,50)
    local currentPresetName = db.presets[db.currentDungeonIdx][db.currentPreset[db.currentDungeonIdx]].text
    NDT.main_frame.ClearConfirmationFrame.label:SetText(string.format(L["Reset %s?"],currentPresetName))
    NDT.main_frame.ClearConfirmationFrame:Show()
end

function NDT:OpenAutomaticColorsDialog()
	NDT:HideAllDialogs()
	NDT.main_frame.automaticColorsFrame:ClearAllPoints()
	NDT.main_frame.automaticColorsFrame:SetPoint("CENTER", NDT.main_frame,"CENTER",0,50)
	NDT.main_frame.automaticColorsFrame:SetStatusText("")
	NDT.main_frame.automaticColorsFrame:Show()
    NDT.main_frame.automaticColorsFrame.CustomColorFrame:Hide()
    if db.colorPaletteInfo.colorPaletteIdx == 6 then
        NDT:OpenCustomColorsDialog()
    end
end

function NDT:OpenCustomColorsDialog(frame)
	NDT:HideAllDialogs()
    NDT.main_frame.automaticColorsFrame:Show() --Not the prettiest way to handle this, but it works.
	NDT.main_frame.automaticColorsFrame.CustomColorFrame:ClearAllPoints()
    NDT.main_frame.automaticColorsFrame.CustomColorFrame:SetPoint("CENTER",264,-7)
	NDT.main_frame.automaticColorsFrame.CustomColorFrame:SetStatusText("")
	NDT.main_frame.automaticColorsFrame.CustomColorFrame:Show()
end

function NDT:UpdateDungeonDropDown()
	local group = NDT.main_frame.DungeonSelectionGroup
...
function NDT:CreateDungeonSelectDropdown(frame)
	...
	group.DungeonDropdown:SetCallback("OnValueChanged",function(widget,callbackName,key)
        if key == 14 then
            ...
            NDT:UpdateDungeonDropDown()
            NDT:UpdateToDungeon(db.currentDungeonIdx)
        elseif key == 27 then
...
function NDT:EnsureDBTables()
    ...
    local preset = NDT:GetCurrentPreset()
    preset.week = preset.week or NDT:GetCurrentAffixWeek()
    ...
    for pullIdx,pull in pairs(preset.value.pulls) do
        for enemyIdx,clones in pairs(pull) do
            ...
            if tonumber(enemyIdx) then
                ...
                if not NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx] then
                    pull[enemyIdx] = nil
                else
                    --only clones
                    for k,v in pairs(clones) do
                        if not NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["clones"][v] then
                            clones[k] = nil
                        end
                    end
                end
    ...
    NDT:GetCurrentPreset().week = NDT:GetCurrentPreset().week or NDT:GetCurrentAffixWeek()
    ...
    local maxSublevel = -1
    for _,_ in pairs(NDT.dungeonMaps[db.currentDungeonIdx]) do
    ...
    preset.value.teeming = NDT:IsWeekTeeming(preset.week)
end

function NDT:GetTileFormat(dungeonIdx)
    local mapInfo = NDT.mapInfo[dungeonIdx]
    return mapInfo and mapInfo.tileFormat or 4
end

function NDT:UpdateMap(ignoreSetSelection, ignoreReloadPullButtons, ignoreUpdateProgressBar)
	local mapName
	local frame = NDT.main_frame
	mapName = NDT.dungeonMaps[db.currentDungeonIdx][0]
	NDT:EnsureDBTables()
    local preset = NDT:GetCurrentPreset()
	...
	local fileName = NDT.dungeonMaps[db.currentDungeonIdx][preset.value.currentSublevel]
	local path = "Interface\\WorldMap\\"..mapName.."\\"
    local tileFormat = NDT:GetTileFormat(db.currentDungeonIdx)
	for i=1,12 do
	...
	if (db.currentPreset[db.currentDungeonIdx] == 1 or db.currentPreset[db.currentDungeonIdx] == presetCount) or NDT.liveSessionActive then
		NDT.main_frame.sidePanelDeleteButton:SetDisabled(true)
		NDT.main_frame.sidePanelDeleteButton.text:SetTextColor(0.5,0.5,0.5)
	else
		NDT.main_frame.sidePanelDeleteButton:SetDisabled(false)
    ...
    --live mode
    local livePreset = NDT:GetCurrentLivePreset()
    if NDT.liveSessionActive and preset ~= livePreset then
        NDT.main_frame.liveReturnButton:Show()
        NDT.main_frame.setLivePresetButton:Show()
    else
        NDT.main_frame.liveReturnButton:Hide()
        NDT.main_frame.setLivePresetButton:Hide()
    end
    NDT:UpdatePresetDropdownTextColor()

	if not ignoreSetSelection then NDT:SetSelectionToPull(preset.value.currentPull) end
	NDT:UpdateDungeonDropDown()
    --frame.sidePanel.affixDropdown:SetAffixWeek(NDT:GetCurrentPreset().week,ignoreReloadPullButtons,ignoreUpdateProgressBar)
    frame.sidePanel.affixDropdown:SetValue(NDT:GetCurrentPreset().week)
    NDT:ToggleFreeholdSelector(db.currentDungeonIdx == 16)
    NDT:ToggleBoralusSelector(db.currentDungeonIdx == 19)
...
function NDT:UpdateToDungeon(dungeonIdx, ignoreUpdateMap, init)
    db.currentExpansion = 1
	...
	NDT:UpdatePresetDropDown()
	if not ignoreUpdateMap then NDT:UpdateMap() end
    NDT:ZoomMapToDefault()
     --Colors the first pull in "Default" presets
    if db.currentPreset[db.currentDungeonIdx] == 1 then NDT:ColorPull() end
end

function NDT:DeletePreset(index)
...
function NDT:CheckCurrentZone(init)
    local zoneId = C_Map.GetBestMapForUnit("player")
    local dungeonIdx = NDT.zoneIdToDungeonIdx[zoneId]
...
function NDT:ClearPreset(preset, silent)
...
function NDT:CreateNewPreset(name)
	if name == "<New Preset>" then
		NDT.main_frame.presetCreationLabel:SetText(string.format(L["Cannot create preset '%s'"],name))
		NDT.main_frame.presetCreationCreateButton:SetDisabled(true)
		NDT.main_frame.presetCreationCreateButton.text:SetTextColor(0.5,0.5,0.5)
		NDT.main_frame.presetCreationFrame:DoLayout()
		return
	end
	...
	if duplicate == false then
		...
		local startingPointPresetIdx = NDT.main_frame.PresetCreationDropDown:GetValue()-1
		if startingPointPresetIdx>0 then
			db.presets[db.currentDungeonIdx][countPresets] = NDT:CopyObject(db.presets[db.currentDungeonIdx][startingPointPresetIdx])
		...
		NDT.main_frame.presetCreationLabel:SetText(string.format(L["Preset '%s' already exists"],name))
		NDT.main_frame.presetCreationCreateButton:SetDisabled(true)
		NDT.main_frame.presetCreationCreateButton.text:SetTextColor(0.5,0.5,0.5)
		NDT.main_frame.presetCreationFrame:DoLayout()
	end
...
function NDT:SanitizePresetName(text)
...
function NDT:MakeChatPresetImportFrame(frame)
    ...
    importButton:SetCallback("OnClick", function()
        ...
        if NDT:ValidateImportPreset(newPreset) then
            chatImport:Hide()
            NDT:ImportPreset(NDT:DeepCopy(newPreset))
        else
            print(L["NDT: Error importing preset"])
        end
    end)
...
function NDT:OpenChatImportPresetDialog(sender, preset, live)
    NDT:HideAllDialogs()
    local chatImport = NDT.main_frame.chatPresetImportFrame
    chatImport:ClearAllPoints()
    chatImport:SetPoint("CENTER", NDT.main_frame,"CENTER",0,50)
...
function NDT:MakePresetImportFrame(frame)
	...
	importButton:SetWidth(100)
	importButton:SetCallback("OnClick", function()
		local newPreset = NDT:StringToTable(importString, true)
		if NDT:ValidateImportPreset(newPreset) then
			NDT.main_frame.presetImportFrame:Hide()
			NDT:ImportPreset(newPreset)
...
function NDT:MakePresetCreationFrame(frame)
	...
	frame.PresetCreationEditbox:SetCallback("OnEnterPressed", function(widget, event, text)
		--check if name is valid, block button if so, unblock if valid
		if NDT:SanitizePresetName(text) then
...
function NDT:ValidateImportPreset(preset)
...
function NDT:ImportPreset(preset, fromLiveSession)
...
---Stores r g b values for coloring pulls with NDT:ColorPull()
...
function NDT:SetPresetColorPaletteInfo()
    local preset = NDT:GetCurrentPreset()
    ...
    --NDT:GetCurrentPreset().colorPaletteInfo = db.colorPaletteInfo
end

---GetPresetColorPaletteInfo
function NDT:GetPresetColorPaletteInfo(preset)
...
function NDT:ColorPull(colorValues, pullIdx, preset, bypass, exportColorBlind) -- bypass can be passed as true to color even when automatic coloring is toggled off
    local colorPaletteInfo = NDT:GetPresetColorPaletteInfo(preset)
    local pullIdx = pullIdx or NDT:GetCurrentPull()
    if(pullIdx) then
        local colorValues
        local numberColors
        local r,g,b
...
function NDT:ColorAllPulls(colorValues, startFrom, bypass, exportColorBlind)
    local preset = self:GetCurrentPreset()
    local startFrom = startFrom or 0
    for pullIdx,_ in pairs(preset.value.pulls) do
        if pullIdx >= startFrom then
            NDT:ColorPull(colorValues, pullIdx, preset, bypass, exportColorBlind)
        end
    end
end
...
function NDT:MakeCustomColorFrame(frame)
    ...
    frame.CustomColorFrame.ColorSlider:SetCallback("OnMouseUp", function(event, callbackName, value)
        ...
        NDT:MakeCustomColorFrame(frame)
        NDT:OpenCustomColorsDialog()
    end)
...
function NDT:MakeAutomaticColorsFrame(frame)
...
function NDT:MakePullSelectionButtons(frame)
...
function NDT:PresetsAddPull(index, data, preset)
    preset = preset or self:GetCurrentPreset()
...
---
---@author Dradux
function NDT:PresetsMergePulls(pulls, destination)
...
function NDT:PresetsDeletePull(p, preset)
    preset = preset or self:GetCurrentPreset()
...
function NDT:GetPulls(preset)
    preset = preset or self:GetCurrentPreset()
    return preset.value.pulls
end

function NDT:GetPullsNum(preset)
    preset = preset or self:GetCurrentPreset()
    return table.getn(preset.value.pulls)
end

function NDT:CopyObject(obj, seen)
...
function NDT:PresetsSwapPulls(p1, p2)
...
function NDT:SetMapSublevel(pull)
	--set map sublevel
	...
	for enemyIdx,clones in pairs(db.presets[db.currentDungeonIdx][db.currentPreset[db.currentDungeonIdx]].value.pulls[pull]) do
        if tonumber(enemyIdx) then
            for idx,cloneIdx in pairs(clones) do
                if NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["clones"][cloneIdx] then
                    lastSubLevel = NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["clones"][cloneIdx].sublevel
                end
            end
        end
	end
	if lastSubLevel then
        ...
        if shouldResetZoom then
            NDT:UpdateMap(true,true,true)
        end
	end

	NDT:UpdateDungeonDropDown()
    if shouldResetZoom then NDT:ZoomMapToDefault() end
end

function NDT:SetSelectionToPull(pull)
	--if pull is not specified set pull to last pull in preset (for adding new pulls)
	if not pull then
		local count = 0
		for k,v in pairs(NDT:GetCurrentPreset().value.pulls) do
			count = count + 1
		end
		pull = count
	end
    ...
    if type(pull) == "number" and pull > 0 then
        NDT:GetCurrentPreset().value.currentPull = pull
        NDT:GetCurrentPreset().value.selection = { pull }
        NDT:PickPullButton(pull)

        NDT:DungeonEnemies_UpdateSelected(pull)
    elseif type(pull) == "table" then
        NDT:GetCurrentPreset().value.currentPull = pull[#pull]
        NDT:GetCurrentPreset().value.selection = pull

        NDT:ClearPullButtonPicks()
        for _, pullIdx in ipairs(NDT:GetSelection()) do
            NDT:PickPullButton(pullIdx, true)
            NDT:DungeonEnemies_UpdateSelected(pullIdx)
        end
    end
end
...
function NDT:UpdatePullButtonNPCData(idx)
    if db.devMode then return end
	local preset = NDT:GetCurrentPreset()
	local frame = NDT.main_frame.sidePanel
	local enemyTable = {}
	if preset.value.pulls[idx] then
		...
		for enemyIdx,clones in pairs(preset.value.pulls[idx]) do
            if tonumber(enemyIdx) then
                ...
                if NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx] then
                    local incremented = false
                    local npcId = NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["id"]
                    local name = NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["name"]
                    local creatureType = NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["creatureType"]
                    local level = NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["level"]
                    local baseHealth = NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["health"]
                    for k,cloneIdx in pairs(clones) do
                        ...
                        if NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["clones"][cloneIdx] then
                            if self:IsCloneIncluded(enemyIdx,cloneIdx) then
                                ...
                                enemyTable[enemyTableIdx].count = NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["count"]
                                enemyTable[enemyTableIdx].teemingCount = NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["teemingCount"]
                                enemyTable[enemyTableIdx].displayId = NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["displayId"]
                                ...
                                enemyTable[enemyTableIdx].ignoreFortified = NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["ignoreFortified"]
                            end
                        end
                    end
                end
    ...
    local pullForces = NDT:CountForces(idx,false)
    local totalForcesMax = NDT:IsCurrentPresetTeeming() and NDT.dungeonTotalCount[db.currentDungeonIdx].teeming or NDT.dungeonTotalCount[db.currentDungeonIdx].normal
    ...
    if idx == 1 then
        ...
        oldPullForces =  NDT:CountForces(idx-1,false)
    end
...
function NDT:ReloadPullButtons()
	local frame = NDT.main_frame.sidePanel
    ...
    for k,pull in ipairs(preset.value.pulls) do
        idx = idx+1
        frame.newPullButtons[idx] = AceGUI:Create("NDTPullButton")
        frame.newPullButtons[idx]:SetMaxPulls(maxPulls)
        frame.newPullButtons[idx]:SetIndex(idx)
        NDT:UpdatePullButtonNPCData(idx)
	...
	frame.newPullButton = AceGUI:Create("NDTNewPullButton")
...
function NDT:ClearPullButtonPicks()
	local frame = NDT.main_frame.sidePanel
...
function NDT:PickPullButton(idx, keepPicked)
    if db.devMode then return end
...
function NDT:AddPull(index)
	NDT:PresetsAddPull(index)
	NDT:ReloadPullButtons()
	NDT:SetSelectionToPull(index)
    NDT:ColorPull()
    NDT:DrawAllHulls()
end

function NDT:SetAutomaticColor(index)
...
function NDT:UpdateAutomaticColors(index)
	if not db.colorPaletteInfo.autoColoring then return end
...
function NDT:ClearPull(index)
    ...
    NDT:ColorPull()
    NDT:DrawAllHulls()
	--NDT:SetAutomaticColor(index)
end

---Moves the selected pull up
function NDT:MovePullUp(index)
	NDT:PresetsSwapPulls(index,index-1)
	NDT:ReloadPullButtons()
	NDT:SetSelectionToPull(index-1)
    NDT:ColorAllPulls(_, index-1)
    NDT:DrawAllHulls()
	--NDT:UpdateAutomaticColors(index - 1)
end

---Moves the selected pull down
function NDT:MovePullDown(index)
	NDT:PresetsSwapPulls(index,index+1)
	NDT:ReloadPullButtons()
	NDT:SetSelectionToPull(index+1)
    NDT:ColorAllPulls(_, index)
    NDT:DrawAllHulls()
	--NDT:UpdateAutomaticColors(index)
end

---Deletes the selected pull and makes sure that a pull will be selected afterwards
function NDT:DeletePull(index)
...
---RenamePreset
function NDT:RenamePreset(renameText)
...
function NDT:GetFirstNotSelectedPullButton(start, direction)
    ...
    local pullIdx = start
    while NDT.U.contains(NDT:GetCurrentPreset().value.selection, pullIdx)
            and NDT.U.isInRange(pullIdx, 1, #NDT:GetCurrentPreset().value.pulls) do
       pullIdx = pullIdx + direction
    end

    if not NDT.U.isInRange(pullIdx, 1, #NDT:GetCurrentPreset().value.pulls) then
        return
    end
...
function NDT:MakeRenameFrame(frame)
	...
	frame.RenameFrame.RenameButton:SetCallback("OnClick",function() NDT:RenamePreset(renameText) end)
...
function NDT:MakeExportFrame(frame)
...
function NDT:MakeDeleteConfirmationFrame(frame)
	...
	frame.DeleteConfirmationFrame.OkayButton:SetCallback("OnClick",function()
		NDT:DeletePreset(db.currentPreset[db.currentDungeonIdx])
...
function NDT:MakeClearConfirmationFrame(frame)
	...
	frame.ClearConfirmationFrame.OkayButton:SetCallback("OnClick",function()
        ...
        if self.liveSessionActive and self:GetCurrentPreset().uid == self.livePresetUID then NDT:LiveSession_SendCommand("clear") end
...
function NDT:OpenConfirmationFrame(width, height, title, buttonText, prompt, callback, buttonText2, callback2)
    local f = NDT.main_frame.ConfirmationFrame
    if not f then
        NDT.main_frame.ConfirmationFrame = AceGUI:Create("Frame")
        f = NDT.main_frame.ConfirmationFrame
    ...
    NDT:HideAllDialogs()
    f:ClearAllPoints()
    f:SetPoint("CENTER", NDT.main_frame,"CENTER",0,50)
...
function NDT:CreateTutorialButton(parent)
    local scale = self:GetScale()
    local sidePanelHeight = NDT.main_frame.sidePanel.PullButtonScrollGroup.frame:GetHeight()
    local helpPlate = {
        FramePos = { x = 0,	y = 0 },
    ...
    if not parent.HelpButton then
        parent.HelpButton = CreateFrame("Button","NDTMainHelpPlateButton",parent,"MainHelpPlateButton")
    ...
    local function TutorialButtonOnClick(self)
        if not HelpPlate_IsShowing(helpPlate) then
            HelpPlate_Show(helpPlate, NDT.main_frame, self)
        else
            HelpPlate_Hide(true)
        end
    end
...
function NDT:DeepCopy(orig)
    local orig_type = type(orig)
    local copy
    if orig_type == 'table' then
        copy = {}
        for orig_key, orig_value in next, orig, nil do
            copy[NDT:DeepCopy(orig_key)] = NDT:DeepCopy(orig_value)
        end
        setmetatable(copy, NDT:DeepCopy(getmetatable(orig)))
...
function NDT:StorePresetObject(obj, ignoreScale, preset)
    --adjust scale
...
function NDT:UpdatePresetObjectOffsets(idx, x, y, preset, silent)
...
function NDT:DrawAllPresetObjects()
...
function NDT:DrawPresetObject(obj, objectIndex, scale, currentPreset, currentSublevel)
...
function NDT:DeletePresetObjects(preset, silent)
	preset = preset or self:GetCurrentPreset()
    if preset == self:GetCurrentPreset() then silent = false end
    local currentSublevel = self:GetCurrentSubLevel()
    for objectIndex,obj in pairs(preset.objects) do
...
function NDT:PresetObjectStepBack(preset, silent)
    preset = preset or self:GetCurrentPreset()
    if preset == self:GetCurrentPreset() then silent = false end
    preset.objects = preset.objects or {}
    local length = 0
...
function NDT:PresetObjectStepForward(preset, silent)
    preset = preset or NDT:GetCurrentPreset()
    if preset == self:GetCurrentPreset() then silent = false end
    preset.objects = preset.objects or {}
    local length = 0
...
function NDT:FixAceGUIShowHide(widget, frame, isFrame, hideOnly)
...
function NDT:GetCurrentAffixWeek()
...
function NDT:PrintCurrentAffixes()
    --run this once so blizz stuff is loaded
    NDT:GetCurrentAffixWeek()
    --https://www.wowhead.com/affixes
    local affixNames = {
        [1] =L["Overflowing"],
        [2] =L["Skittish"],
...
function NDT:IsPlayerInGroup()
    local inGroup = (UnitInRaid("player") and "RAID") or (IsInGroup() and "PARTY")
    return inGroup
end

function NDT:ResetMainFramePos(soft)
...
function NDT:DropIndicator()
    local indicator = NDT.main_frame.drop_indicator
    if not indicator then
        indicator = CreateFrame("Frame", "NDT_DropIndicator")
        ...
        NDT.main_frame.drop_indicator = indicator
...
function NDT:IsShown_DropIndicator()
    local indicator = NDT:DropIndicator()
    return indicator:IsShown()
end

function NDT:Show_DropIndicator(target, pos)
    local indicator = NDT:DropIndicator()
...
function NDT:Hide_DropIndicator()
    local indicator = NDT:DropIndicator()
    indicator:Hide()
end

function NDT:GetSelection()
    if not NDT:GetCurrentPreset().value.selection or #NDT:GetCurrentPreset().value.selection == 0 then
        NDT:GetCurrentPreset().value.selection = { NDT:GetCurrentPreset().value.currentPull }
    end

    return NDT:GetCurrentPreset().value.selection
end

function NDT:GetScrollingAmount(scrollFrame, pixelPerSecond)
    local viewheight = scrollFrame.frame.obj.content:GetHeight()
    return (pixelPerSecond / viewheight) * 1000
end

function NDT:ScrollToPull(pullIdx)
    -- Get scroll frame
    ...
    local pulls = #NDT:GetCurrentPreset().value.pulls + 1 or 1
...
function NDT:CopyPullOptions(sourceIdx, destinationIdx)
    local preset = NDT:GetCurrentPreset()
...
function NDT:GetPullButton(pullIdx)
    local frame = NDT.main_frame.sidePanel
    return frame.newPullButtons[pullIdx]
end

function NDT:UpdatePullButtonColor(pullIdx, r, g, b)
...
--/run NDT:ResetDataCache();
function NDT:ResetDataCache()
...
function initFrames()
    local main_frame = CreateFrame("frame", "NDTFrame", UIParent)
    tinsert(UISpecialFrames,"NDTFrame")

    --cache dungeon data to not lose data during reloads
    if db.devMode then
        if db.dungeonEnemies then
            NDT.dungeonEnemies = db.dungeonEnemies
        else
            db.dungeonEnemies = NDT.dungeonEnemies
        end
        if db.mapPOIs then
            NDT.mapPOIs = db.mapPOIs
        else
            db.mapPOIs = NDT.mapPOIs
        end
    end
	...
	main_frame.background:SetColorTexture(unpack(NDT.BackdropColor))
    ...
    local _,_,fullscreenScale = NDT:GetFullScreenSizes()
    main_frame:SetMaxResize(sizex*fullscreenScale,sizey*fullscreenScale)
	NDT.main_frame = main_frame
    ...
    main_frame.mainFrametex:SetColorTexture(unpack(NDT.BackdropColor))
    ...
    main_frame.contextDropdown = L_Create_UIDropDownMenu("NDTContextDropDown", nil)

    NDT:CheckCurrentZone(true)
    NDT:EnsureDBTables()
	NDT:MakeTopBottomTextures(main_frame)
	NDT:MakeMapTexture(main_frame)
	NDT:MakeSidePanel(main_frame)
    NDT:CreateMenu()
	NDT:MakePresetCreationFrame(main_frame)
	NDT:MakePresetImportFrame(main_frame)
    NDT:DungeonEnemies_CreateFramePools()
	--NDT:UpdateDungeonEnemies(main_frame)
	NDT:CreateDungeonSelectDropdown(main_frame)
	NDT:MakePullSelectionButtons(main_frame.sidePanel)
	NDT:MakeExportFrame(main_frame)
	NDT:MakeRenameFrame(main_frame)
	NDT:MakeDeleteConfirmationFrame(main_frame)
	NDT:MakeClearConfirmationFrame(main_frame)
	NDT:CreateTutorialButton(main_frame)
    NDT:POI_CreateFramePools()
    NDT:MakeChatPresetImportFrame(main_frame)
	NDT:MakeSendingStatusBar(main_frame)
	NDT:MakeAutomaticColorsFrame(main_frame)
    NDT:MakeCustomColorFrame(main_frame.automaticColorsFrame)

    --devMode
    if db.devMode and NDT.CreateDevPanel then
        NDT:CreateDevPanel(NDT.main_frame)
    end
    ...
    --tooltip new
    do
        NDT.tooltip = CreateFrame("Frame", "NDTModelTooltip", UIParent, "TooltipBorderedFrameTemplate")
        local tooltip = NDT.tooltip
        ...
        tooltip.String = tooltip:CreateFontString("NDTToolTipString")
	...
	--pullTooltip
	do
		NDT.pullTooltip = CreateFrame("Frame", "NDTPullTooltip", UIParent, "TooltipBorderedFrameTemplate")
        --NDT.pullTooltip:SetOwner(UIParent, "ANCHOR_NONE")
        local pullTT = NDT.pullTooltip
        NDT.pullTooltip:SetClampedToScreen(true)
		NDT.pullTooltip:SetFrameStrata("TOOLTIP")
        NDT.pullTooltip.myHeight = 160
		NDT.pullTooltip:SetSize(250, NDT.pullTooltip.myHeight)
        NDT.pullTooltip.Model = CreateFrame("PlayerModel", nil, NDT.pullTooltip)
        NDT.pullTooltip.Model:SetFrameLevel(1)
        NDT.pullTooltip.Model.fac = 0
        if true then
            NDT.pullTooltip.Model:SetScript("OnUpdate",function (self, elapsed)
                self.fac = self.fac + 0.5
                if self.fac >= 360 then
                    self.fac = 0
                end
            ...
            NDT.pullTooltip.Model:SetPortraitZoom(1)
            NDT.pullTooltip.Model:SetFacing(PI*2 / 360 * 2)
        end

        NDT.pullTooltip.Model:SetSize(110,110)
        NDT.pullTooltip.Model:SetPoint("TOPLEFT", NDT.pullTooltip, "TOPLEFT",7,-7)

        NDT.pullTooltip.topString = NDT.pullTooltip:CreateFontString("NDTToolTipString")
        NDT.pullTooltip.topString:SetFontObject("GameFontNormalSmall")
        NDT.pullTooltip.topString:SetFont(NDT.pullTooltip.topString:GetFont(),10)
        NDT.pullTooltip.topString:SetTextColor(1, 1, 1, 1)
        NDT.pullTooltip.topString:SetJustifyH("LEFT")
        NDT.pullTooltip.topString:SetJustifyV("TOP")
        NDT.pullTooltip.topString:SetHeight(110)
        NDT.pullTooltip.topString:SetWidth(130)
        NDT.pullTooltip.topString:SetPoint("TOPLEFT", NDT.pullTooltip, "TOPLEFT", 110, -7)
        NDT.pullTooltip.topString:Hide()

        local heading = NDT.pullTooltip:CreateTexture(nil, "TOOLTIP")
        heading:SetHeight(8)
        heading:SetPoint("LEFT", 12, -30)
        heading:SetPoint("RIGHT", NDT.pullTooltip, "RIGHT", -12, -30)
        heading:SetTexture("Interface\\Tooltips\\UI-Tooltip-Border")
        heading:SetTexCoord(0.81, 0.94, 0.5, 1)
        heading:Show()

        NDT.pullTooltip.botString = NDT.pullTooltip:CreateFontString("NDTToolTipString")
        local botString = NDT.pullTooltip.botString
        botString:SetFontObject("GameFontNormalSmall")
        botString:SetFont(NDT.pullTooltip.topString:GetFont(),10)
	...
	NDT:initToolbar(main_frame)
    if db.toolbarExpanded then
        main_frame.toolbar.toggleButton:Click()
    end

    --ping
    NDT.ping = CreateFrame("PlayerModel", nil, NDT.main_frame.mapPanelFrame)
    local ping = NDT.ping
    ...
    main_frame.sidePanel.affixDropdown:SetAffixWeek(NDT:GetCurrentPreset().week or (NDT:GetCurrentAffixWeek() or 1))
    NDT:UpdateToDungeon(db.currentDungeonIdx)
	main_frame:Hide()

    --Maximize if needed
    if db.maximized then NDT:Maximize() end
