    ...
    for i=29,36 do
        if db.dataCollection[i] then
            for id,spells in pairs(db.dataCollection[i]) do
                local enemies = NDT.dungeonEnemies[i]
...
function NDT:RequestDataCollectionUpdate()
    --temporary lag fix
    if true then return end
    local distribution = self:IsPlayerInGroup()
    if not distribution then return end
    NDTcommsObject:SendCommMessage(self.dataCollectionPrefixes.request, "0", distribution, nil, "ALERT")
end
...
function DC:DistributeData()
    ...
    if not DC.lastDistribution or DC.lastDistribution < GetTime() - 300 then
        ...
        local export = NDT:TableToString(package,false,5)
        NDTcommsObject:SendCommMessage(NDT.dataCollectionPrefixes.distribute, export, distribution, nil, "BULK",nil,nil)
    end
end
...
function DC:InitHealthTrack()
    ...
    for _,enemy in pairs(NDT.dungeonEnemies[db.currentDungeonIdx]) do
    ...
    f:SetScript("OnEvent", function(self, event, ...)
        if event == "UPDATE_MOUSEOVER_UNIT" then
            ...
            if npcId then
                ...
                for enemyIdx,enemy in pairs(NDT.dungeonEnemies[db.currentDungeonIdx]) do
                    if enemy.id == tonumber(npcId) then
                        if enemy.health ~= npcHealth then
                            ...
                            local enemiesLeft = " "
                            enemiesToScale = {}
                            for k,v in pairs(NDT.dungeonEnemies[db.currentDungeonIdx]) do
...
function NDT:FinishHPTrack()
    ...
    for enemyIdx in pairs(enemiesToScale) do
        local enemy = NDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]
