...
3. Call NDT:AddNPCFromUnit("mouseover") with macro to add untargetable units
...
function NDT:AddNPCFromUnit(unit)
    db = NDT:GetDB()
    local npcId
    ...
    local added
    for _,npcData in pairs(NDT.dungeonEnemies[db.currentDungeonIdx]) do
    ...
    if npcId and not added then
        ...
        local npcScale = 1
        local npcCount = 0
        tinsert(NDT.dungeonEnemies[db.currentDungeonIdx], {
            name = npcName,
            health = npcHealth,
            level = npcLevel,
...
function NDT:CreateDevPanel(frame)
    db = NDT:GetDB()
    ...
    NDT:FixAceGUIShowHide(devPanel)
    ...
    local function DrawGroup1(container)
        ...
        local buttons = {
            [1] = {
                text="MapLink",
                func=function()
                    if not NDT.mapPOIs[db.currentDungeonIdx] then NDT.mapPOIs[db.currentDungeonIdx] = {} end
                    if not NDT.mapPOIs[db.currentDungeonIdx][NDT:GetCurrentSubLevel()] then
                        NDT.mapPOIs[db.currentDungeonIdx][NDT:GetCurrentSubLevel()] = {}
                    end
                    local links = NDT.mapPOIs[db.currentDungeonIdx][NDT:GetCurrentSubLevel()]
            ...
            [7] = {
                text="Export to LUA",
                func=function()
                    local export = tshow(NDT.mapPOIs[db.currentDungeonIdx],"NDT.mapPOIs[dungeonIndex]")
                    ...
                    NDT.main_frame.ExportFrameEditbox:HighlightText(0, slen(export))
    ...
    local function DrawGroup2(container)
        ...
        local function updateFields(health,level,creatureType,id,scale,count,idx)
            if idx then
                local data = NDT.dungeonEnemies[db.currentDungeonIdx][idx]
        ...
        local function updateDropdown(npcId,idx)
            if not NDT.dungeonEnemies[db.currentDungeonIdx] then return end
            idx = idx or 1
            local enemies = {}
            for mobIdx,data in ipairs(NDT.dungeonEnemies[db.currentDungeonIdx]) do
        ...
        dropdown:SetCallback("OnValueChanged", function(widget,callbackName,key)
            ...
            local dungeonEnemyBlips = NDT:GetDungeonEnemyBlips()
        ...
        countSlider:SetCallback("OnMouseUp",function(widget,callbackName,value)
            ...
            local data = NDT.dungeonEnemies[db.currentDungeonIdx][npcIdx]
            data["count"] = value
            NDT:UpdateMap()
        end)
        ...
        button1:SetCallback("OnClick",function()
            local npcId = NDT:AddNPCFromUnit("target")
        ...
        button2:SetCallback("OnClick",function()
            local currentBlip = NDT:GetCurrentDevmodeBlip()
            if currentBlip then
                --encounterID
                local encounterID, encounterName, description, displayInfo, iconImage = EJ_GetCreatureInfo(1)
                if not encounterID then
                    print("NDT: Error - Make sure to open Encounter Journal and navigate to the boss you want to add!")
                    return
                end
                for i=1,10000 do
                ...
                local data = NDT.dungeonEnemies[db.currentDungeonIdx][currentBlip.enemyIdx]
                data.isBoss = true
        ...
        button3:SetCallback("OnClick",function()
            NDT:CleanEnemyData(db.currentDungeonIdx)
            local export = tshow(NDT.dungeonEnemies[db.currentDungeonIdx],"NDT.dungeonEnemies[dungeonIndex]")
            NDT.main_frame.ExportFrame:ClearAllPoints()
            NDT.main_frame.ExportFrame:Show()
            NDT.main_frame.ExportFrame:SetPoint("CENTER", NDT.main_frame,"CENTER",0,50)
            NDT.main_frame.ExportFrameEditbox:SetText(export)
    ...
    local function DrawGroup5(container)
        ...
        createSpireButton:SetCallback("OnClick",function()
            ...
            local pois = NDT.mapPOIs[db.currentDungeonIdx][NDT:GetCurrentSubLevel()]
            local posx,posy = 300,-200
            local newWeek = NDT:DeepCopy(week)
            ...
            newWeek = NDT:DeepCopy(week)
            NDT:POI_UpdateAll()
            --add associated NPC to the map
            NDT:AddCloneFromData(tooltipIdxToNpcId[tooltipIndex],newWeek)
        end)
    ...
    local function DrawGroup6(container)
        ...
        resetDbButton:SetCallback("OnClick",function()
            NDT:OpenConfirmationFrame(300, 150, "Reset NDT DB", "Confirm", "Do you want to reset NDT DB?", function()
                NDT:HardReset()
            end, "Cancel", nil)
        end)
    ...
    --hook UpdateMap
    local originalFunc = NDT.UpdateMap
    function NDT:UpdateMap(...)
        originalFunc(...)
        local selectedTab
...
function NDT:AddCloneFromData(npcId, weeks)
    local sublevel= NDT:GetCurrentSubLevel()
    local x,y = 320,-200
    local data
    for _,enemyData in pairs(NDT.dungeonEnemies[db.currentDungeonIdx]) do
    ...
    print(string.format("NDT: Created clone %s %d at %d,%d",data.name,#data.clones,x,y))
    NDT:UpdateMap()
end
...
function NDT:AddCloneAtCursorPosition()
    if not MouseIsOver(NDTScrollFrame) then return end
    if currentEnemyIdx then
        local data = NDT.dungeonEnemies[db.currentDungeonIdx][currentEnemyIdx]
        local cursorx,cursory = NDT:GetCursorPosition()
        local scale = self:GetScale()
        cursorx = cursorx*(1/scale)
        cursory = cursory*(1/scale)
        tinsert(data.clones,{ x=cursorx, y=cursory, sublevel= NDT:GetCurrentSubLevel(), g=currentCloneGroup, teeming=currentTeeming})
        print(string.format("NDT: Created clone %s %d at %d,%d",data.name,#data.clones,cursorx,cursory))
        NDT:UpdateMap()
    end
end
...
function NDT:AddPatrolWaypointAtCursorPosition()
    if not MouseIsOver(NDTScrollFrame) then return end
    local currentBlip = NDT:GetCurrentDevmodeBlip()
    if currentBlip then
        local data = NDT.dungeonEnemies[db.currentDungeonIdx][currentBlip.enemyIdx]
        ...
        local patrolBlips = NDT:GetPatrolBlips()
        ...
        print(string.format("NDT: Created Waypoint %d of %s %d at %d,%d",1,data.name,#cloneData.patrol,cursorx,cursory))
        NDT:UpdateMap()
    end
end
